...
function NotebookIframe({ config, groupId }) {
  ...
  const theRewriteAppSrc = `${config.theRewriteAppUrl}#config=${configParam}`;
 ...
 * @prop {import('../../annotator/util/emitter').EventBus} eventBus
 * @prop {Record<string, any>} config
 */
...
export default function NotebookModal({ eventBus, config }) {
  ...
  const onClose = () => {
    setIsHidden(true);
    emitter.current.publish('closeTheRewrite');
  };
